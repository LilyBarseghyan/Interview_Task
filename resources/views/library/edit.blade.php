        ...
        <div class="card m-3  bg-white">
            <div class="card-body">
                <div class="col-md-12 mb-4">
                    ...
                    <input type="text" class="form-control @if($errors->has('name'))is-invalid @endif" id="name" name="name" value="{{$library['name']}}">
                    @if($errors->has('name'))
                    ...
                    <select class="form-control @if($errors->has('attachments'))is-invalid @endif" name="attachments[]" id="attachments" multiple >
                        @foreach ($books as $book)
                            <option value="{{$book['id']}}"
                                    @if(in_array($book['id'], $attachments)) selected @endif>{{$book['title']}}</option>
                        @endforeach
                    </select>
